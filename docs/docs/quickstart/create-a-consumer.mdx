...
Here, you set some minimal configurations for your web server. By default, the Gosoline expects that there is an input configured with the configuration key `stream.input.consumer`. This defines the input source. In this tutorial, you'll use a file as source with the configured filename, "events.json".
