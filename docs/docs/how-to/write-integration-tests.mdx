...
## Container expiry and cleanup

When Gosoline's test environment starts Docker containers (e.g. Localstack, Kafka, MySQL), it also configures an automatic expiry to ensure resources are cleaned up even if a test crashes.

By default, containers are configured to expire after `5m`:
...
You can override this per suite using `WithContainerExpireAfter`:

```go
func (s *MySuite) SetupSuite() []suite.Option {
    return []suite.Option{
...
Or globally via your configuration:
...
The value is a Go duration string (e.g. `30s`, `5m`, `1h`). The test environment will still stop containers normally when the test finishes; `expire_after` is a safety net to avoid orphaned containers.
...
We define two components of type ddb (for DynamoDB) and mysql (for a MySQL server).
