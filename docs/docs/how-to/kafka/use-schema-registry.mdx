...
Using the shema registry is optional however and gosoline will let you read and write anything if you choose to.

Moreover, keep in mind that the open source Apache Kafka broker does not have any server side schema validation and will still let you write anything to a topic if you do not configure gosoline to use the schema registry,
...
Now you need to provide the schema settings via the `strean.WithSchemaSettings` option when creating your Producer/Publisher.
