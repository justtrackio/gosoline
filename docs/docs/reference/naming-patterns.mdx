...
These placeholders come from `cfg.AppIdentity` and work in all `AppIdentity.Format()` based patterns:
...
- Placeholders must be separated by dots (`.`)
- No static text is allowed
- The model name is appended automatically as the last segment
