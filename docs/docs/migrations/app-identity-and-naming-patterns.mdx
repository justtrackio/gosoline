...
2. The pattern must consist *only* of placeholders separated by **single dots (`.`)**. No other characters or delimiters (like dashes) are allowed between placeholders.
...
| **Delimiter Config** | `conc.ddb.leader_election.<name>.naming.table_delimiter` |
| **Delimiter Default** | `-` |
