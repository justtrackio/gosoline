...
func (s *WriterBlobTestSuite) TestFileReader() {
	...
	config := s.config
	logger := s.logger
	reader, err := blob.NewFileReader(tmpDir)(s.T().Context(), config, logger, "test")
	s.NoError(err)
	...
	go func() {
		defer close(chDoneRead)

		s.NoError(reader.Run(s.T().Context()))
	}()
...
func (s *WriterBlobTestSuite) TestFileReader_WithContextCancellation() {
	...
	go func() {
		...
		s.ErrorIs(reader.Run(ctx), context.Canceled)
	}()
	...
	case _, ok := <-ch:
		if ok {
			s.Fail("should never read an object from channel, as context was cancelled prior reader.Read")
		}
...
func (s *WriterBlobTestSuite) TestNewFileReader_InvalidPath() {
	...
	config := s.config
	logger := s.logger
	reader, err := blob.NewFileReader("/non/existent/path")(s.T().Context(), config, logger, "test")
	s.NoError(err) // NewFileReader doesn't validate path existence
	s.NotNil(reader)

	// Test with relative path - should be converted to absolute
	reader, err = blob.NewFileReader(".")(s.T().Context(), config, logger, "test")
	s.NoError(err)
	s.NotNil(reader)
}
