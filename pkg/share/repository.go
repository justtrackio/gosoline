...
func newRepository(ctx context.Context, config cfg.Config, logger log.Logger) (db_repo.Repository[uint, *Share], error) {
	...
	repo, err := db_repo.New[uint, *Share](ctx, config, logger, dbSettings)
	if err != nil {
...
func (r sqlRepository) Delete(ctx context.Context, value *Share) error {
	...
	err = r.guard.DeletePolicy(ctx, &ladon.DefaultPolicy{ID: value.GetPolicyId()})
	if err != nil {
		return err
	}
