...
func (r *containerRunnerLocal) RunContainer(ctx context.Context, request ContainerRequest) (*Container, error) {
	...
	if err = r.pullContainerImage(request.ContainerDescription); err != nil {
		return nil, fmt.Errorf("could not pull container image %q: %w", err)
	}

	if container, err = r.runNewContainer(ctx, request); err != nil {
...
func (r *containerRunnerLocal) runNewContainer(ctx context.Context, request ContainerRequest) (*Container, error) {
	...
	container, err := r.runContainer(runOptions, resourceId, config.ExpireAfter, config.PortBindings, request.ComponentType, tmpfsConfig)
	if err != nil {
		return nil, err
	}
...
func (r *containerRunnerLocal) runContainer(options *dockertest.RunOptions, resourceId string, expireAfter time.Duration, bindings PortBindings, typ string, tmpfs map[string]string) (*Container, error) {
