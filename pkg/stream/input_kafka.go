	...
	for {
		...
		for _, fetchError := range fetchErrors {
			...
			case errors.As(fetchError.Err, &errDataLoss):
				...
				i.logger.WithContext(ctx).Warn("%s", fetchError.Err.Error())
			default:
				i.logger.WithContext(ctx).Error("failed to fetch records (topic: %s. partition: %d): %w", fetchError.Topic, fetchError.Partition, fetchError.Err)
			}
		}
