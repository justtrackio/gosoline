...
func init() {
	cfg.AddPostProcessor(16, "gosoline.metrics.mpr_metric_leader_elections", mprConfigPostprocessor)
}
...
func mprConfigPostprocessor(config cfg.GosoConf) (bool, error) {
	for _, handler := range handlers {
		...
		if err := config.Option(configOptions...); err != nil {
			return false, fmt.Errorf("can not apply config settings for metrics per runner leader election: %w", err)
		}
