...
func New[K mdl.PossibleIdentifier, M ModelBased[K]](config cfg.Config, logger log.Logger, s Settings) (ConfigurableRepository[K, M], error) {
	...
	return NewWithInterfaces[K, M](logger, tracer, orm, clk, s.Metadata, CreateModel[M]), nil
}

func NewWithDbSettings[K mdl.PossibleIdentifier, M ModelBased[K]](config cfg.Config, logger log.Logger, dbSettings db.Settings, repoSettings Settings) (ConfigurableRepository[K, M], error) {
	tracer, err := tracing.ProvideTracer(config, logger)
	if err != nil {
		return nil, fmt.Errorf("can not create tracer: %w", err)
	}
...
func NewWithInterfaces[K mdl.PossibleIdentifier, M ModelBased[K]](logger log.Logger, tracer tracing.Tracer, orm *gorm.DB, clock clock.Clock, metadata Metadata, modelSource func() M) ConfigurableRepository[K, M] {
...
func (r *repository[K, M]) Create(ctx context.Context, value M) error {
	...
	logger.Info("created model of type %s with id %v", modelId, *value.GetId())
...
func (r *repository[K, M]) Read(ctx context.Context, id K) (M, error) {
	...
	err := r.orm.First(out, map[string]interface{}{
...
func (r *repository[K, M]) Update(ctx context.Context, value M) error {
	...
	if db.IsDuplicateEntryError(err) {
		logger.Warn("could not update model of type %s with id %v due to duplicate entry error: %s", modelId, mdl.EmptyIfNil(value.GetId()), err.Error())
	...
	logger.Info("updated model of type %s with id %v", modelId, *value.GetId())
...
func (r *repository[K, M]) Delete(ctx context.Context, value M) error {
	...
	logger.Info("deleted model of type %s with id %v", modelId, *value.GetId())
