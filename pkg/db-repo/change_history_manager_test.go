...
func (s *changeHistoryManagerTestSuite) TestRunMigration_Disabled() {
	...
	s.logger.EXPECT().Info("planned schema change: CREATE TABLE `test_models_history` (`change_history_action`  VARCHAR(8) NOT NULL DEFAULT 'insert',`change_history_action_at` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,`change_history_revision` int ,`change_history_author_id` int,`id` int unsigned ,`updated_at` datetime,`created_at` datetime,`value` varchar(255), PRIMARY KEY (`change_history_revision`,`id`))").Once()
	...
	s.logger.EXPECT().Info("planned schema change: CREATE TRIGGER test_models_history_revai BEFORE INSERT ON `test_models_history` FOR EACH ROW SET NEW.change_history_revision = (SELECT IFNULL(MAX(d.change_history_revision), 0) + 1 FROM `test_models_history` as d WHERE d.`id` = NEW.`id`);").Once()
