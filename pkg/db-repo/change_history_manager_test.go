...
func (s *changeHistoryManagerTestSuite) TestRunMigration_Disabled() {
	...
	s.logger.EXPECT().Info("change history migration is disabled").Once()
	...
	s.logger.EXPECT().Info("planned schema change: CREATE TRIGGER test_models_history_revai BEFORE INSERT ON `test_models_history` FOR EACH ROW \n\t\t\tSET NEW.change_history_revision = (SELECT IFNULL(MAX(d.change_history_revision), 0) + 1 FROM `test_models_history` as d WHERE d.`id` = NEW.`id`);").Once()                                                                                                                                                                                                                                                                                                       //nolint:lll // test scenario
	...
	s.Require().Error(err)
	s.Equal("cannot execute change history migration: change history migration disabled", err.Error())
}
