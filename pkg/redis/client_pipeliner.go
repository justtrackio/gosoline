...
func (p *prefixedPipeliner) HGetAll(ctx context.Context, key string) *baseRedis.StringStringMapCmd {
	key, err := p.client.prefixKey(key)
	if err != nil {
		cmd := baseRedis.NewStringStringMapCmd(ctx, "hgetall")
		cmd.SetErr(err)
...
func (p *prefixedPipeliner) ZRandMember(ctx context.Context, key string, count int, withScores bool) *baseRedis.StringSliceCmd {
	key, err := p.client.prefixKey(key)
	if err != nil {
	...
	return p.Pipeliner.ZRandMember(ctx, key, count, withScores)
}
