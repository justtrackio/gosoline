...
func assertAttachmentMessage(t testing.TB, reader io.Reader, from string, recipients []string, subject string, textBody string, htmlBody string, attachment *email.Attachment) {
	t.Helper()
	...
	require.NoError(t, err)
	require.Equal(t, from, sender.Address)
	...
	for index, recipient := range recipientAddresses {
		require.Equal(t, recipients[index], recipient.Address)
	}
