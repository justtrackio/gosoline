...
func AddCrudHandlers[CI any, UI any, O any, K mdl.PossibleIdentifier, M dbRepo.ModelBased[K]](logger log.Logger, d *httpserver.Definitions, version int, basePath string, handler Handler[CI, UI, O, K, M]) {
...
func AddCreateHandler[I any, O any, K mdl.PossibleIdentifier, M dbRepo.ModelBased[K]](logger log.Logger, d *httpserver.Definitions, version int, basePath string, handler CreateHandler[I, O, K, M]) {
...
func AddUpdateHandler[I any, O any, K mdl.PossibleIdentifier, M dbRepo.ModelBased[K]](logger log.Logger, d *httpserver.Definitions, version int, basePath string, handler UpdateHandler[I, O, K, M]) {
	_, idPath := getHandlerPaths(version, basePath)

	d.PUT(idPath, NewUpdateHandler(logger, handler))
}

func AddDeleteHandler[O any, K mdl.PossibleIdentifier, M dbRepo.ModelBased[K]](logger log.Logger, d *httpserver.Definitions, version int, basePath string, handler BaseHandler[O, K, M]) {
	_, idPath := getHandlerPaths(version, basePath)
