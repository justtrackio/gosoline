...
func (_m *Repository) Read(ctx context.Context, id *uint64, out db_repo.ModelBased) error {
	ret := _m.Called(ctx, id, out)

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, *uint64, db_repo.ModelBased) error); ok {
		r0 = rf(ctx, id, out)
	} else {
		r0 = ret.Error(0)
	}
