...
func TestCountingBodyReader(t *testing.T) {
	r, readBytes := apiserver.NewCountingBodyReader(io.NopCloser(bytes.NewReader([]byte{1, 2, 3, 4})))
...
func TestCountingBodyWriter(t *testing.T) {
	rw := &responseWriter{}
	w, writtenBytes := apiserver.NewCountingBodyWriter(rw)
...
func TestGZipBodyReader(t *testing.T) {
	...
	reader, readUncompressedBytes, err := apiserver.NewGZipBodyReader(io.NopCloser(bytes.NewReader(compressedBody)))
	assert.NoError(t, err)
