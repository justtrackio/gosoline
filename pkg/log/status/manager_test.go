...
func TestManager(t *testing.T) {
	...
	logger.EXPECT().Info("Work item %s: step %d / %d (%.2f %%)", "test", 0, 3, 0.0).Once()
	m.PrintReport(logger)

	h.ReportProgress(1, 50)
	logger.EXPECT().Info("Work item %s: step %d / %d (%.2f %%)", "test", 1, 3, 50.0).Once()
	m.PrintReport(logger)

	h.ReportDone()
	logger.EXPECT().Info("Work item %s: done", "test").Once()
	m.PrintReport(logger)

	h = m.StartWork("next test", 1)
	logger.EXPECT().Info("Work item %s: step %d / %d (%.2f %%)", "next test", 0, 1, 0.0).Once()
	logger.EXPECT().Info("Work item %s: done", "test").Once()
	m.PrintReport(logger)

	h.ReportError(fmt.Errorf("out of ideas"))
	logger.EXPECT().Info("Work item %s: failed with error %s", "next test", "out of ideas").Once()
	logger.EXPECT().Info("Work item %s: done", "test").Once()
	m.PrintReport(logger)
	...
	logger.EXPECT().Info("Work item %s: failed with error %s", "monitored error", "error: success").Once()
	logger.EXPECT().Info("Work item %s: failed with error %s", "monitored panic", err.Error()).Once()
	logger.EXPECT().Info("Work item %s: failed with error %s", "next test", "out of ideas").Once()
	logger.EXPECT().Info("Work item %s: done", "test").Once()
	m.PrintReport(logger)
}
