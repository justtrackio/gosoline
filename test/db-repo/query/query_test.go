...
func (s *DbRepoQueryTestSuite) TestCreateCorrectModel() {
	...
	repo, err := db_repo.New[uint, *TestModel](envConfig, envLogger, db_repo.Settings{
	...
	models, err := repo.Query(context.Background(), qb)
	s.NoError(err)
...
func (s *DbRepoQueryTestSuite) TestCreateWrongModel() {
	...
	repo, err := db_repo.New[uint, *WrongTestModel](envConfig, envLogger, db_repo.Settings{
		Metadata: TestModelMetadata,
	})
	s.NoError(err)
...
func (s *DbRepoQueryTestSuite) TestReadCorrectModel() {
	...
	readModel, err := repo.Read(context.Background(), *model.GetId())
	s.NoError(err)
...
func (s *DbRepoQueryTestSuite) TestReadWrongModel() {
	...
	_, err = repo.Read(context.Background(), 1)
...
func (s *DbRepoQueryTestSuite) TestQueryWrongModel() {
	...
	_, err = repo.Query(context.Background(), qb)
