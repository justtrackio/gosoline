...
func (s *ChangeHistoryTestSuite) SetupSuite() []suite.Option {
	return []suite.Option{
		...
		suite.WithModule("default", moduleFactory),
		suite.WithDbRepoChangeHistory(),
		suite.WithClockProviderAt("2024-01-01T00:00:00Z"),
		suite.WithContainerExpireAfter(time.Minute * 2),
...
func (s *ChangeHistoryTestSuite) assertHistory(entityId uint, expected []ItemsHistory) {
	repo, err := NewHistoryRepository(s.Env().Context(), s.Env().Config(), s.Env().Logger())
	if !s.NoError(err) {
		return
	}

	histories := make([]*ItemsHistory, 0)
	...
	items := funk.Map(histories, func(h *ItemsHistory) ItemsHistory {
